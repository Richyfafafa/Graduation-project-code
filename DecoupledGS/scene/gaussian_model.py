...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        self.inverse_mirror_activation = inverse_sigmoid
    ...
    def __init__(self, sh_degree = -1):
        ...
        self._mirror_weight = torch.empty(0)
        ...
        self.use_mirror_for_prop = False
        self.mirror_prop_thr = 0.5
    ...
    def restore(self, model_args, training_args):
        ...
        if len(model_args) == 13:
            ...
            mirror_init = self.inverse_mirror_activation(0.01 * torch.ones((self._xyz.shape[0], 1), device="cuda"))
            self._mirror_weight = nn.Parameter(mirror_init.requires_grad_(True))
        else:
            (self.active_sh_degree,
            self._xyz,
            self._scaling,
    ...
    @property
    def get_mirror_weight(self):
        return self.mirror_activation(self._mirror_weight)
    ...
    def get_min_axis(self, cam_o):
        pts = self.get_xyz
        if cam_o.device != pts.device:
            cam_o = cam_o.to(pts.device)
        p2o = cam_o[None] - pts
    ...
    def init_properties_from_pcd(self, pts, colors):
        ...
        mirror = self.inverse_mirror_activation(torch.ones_like(opacities).cuda() * 0.01)
        return {
            'opac': opacities, 'rot':rots, 'scale':scales, 'shs':features, 'refl':refl, 'mirror': mirror
        }
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale: float, cubemap_resol = 128):
        ...
        self._mirror_weight = nn.Parameter(tot_props['mirror'].requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        self.use_mirror_for_prop = bool(getattr(training_args, "use_mirror_for_prop", False))
        self.mirror_prop_thr = float(getattr(training_args, "mirror_prop_thr", 0.5))
        ...
        l = [
            ...
            {'params': [self._mirror_weight], 'lr': training_args.mirror_lr, "name": "mirror"},
    ...
    def save_ply(self, path):
        ...
        mirror_weight = self._mirror_weight.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, refls, mirror_weight, scale, rotation), axis=1)
    ...
    def get_prop_refl_mask(self, refl_thr):
        refl_msk = self.get_refl.flatten() > refl_thr
        if not self.use_mirror_for_prop:
            return refl_msk
        mirror_msk = self.get_mirror_weight.flatten() > self.mirror_prop_thr
        return torch.logical_or(refl_msk, mirror_msk)

    def dist_rot(self): #
        REFL_MSK_THR = 0.1
        refl_msk = self.get_prop_refl_mask(REFL_MSK_THR)
    ...
    def load_ply(self, path):
        ...
        mirror_prop = plydata.elements[0].data.dtype.names
        if mirror_prop is not None and "mirror" in mirror_prop:
            mirror_weight = np.asarray(plydata.elements[0]["mirror"])[..., np.newaxis]
        else:
            mirror_weight = None
        ...
        if mirror_weight is None:
            mirror_init = self.inverse_mirror_activation(0.01 * torch.ones((xyz.shape[0], 1), dtype=torch.float, device="cuda"))
            self._mirror_weight = nn.Parameter(mirror_init.requires_grad_(True))
        else:
            self._mirror_weight = nn.Parameter(torch.tensor(mirror_weight, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._mirror_weight = optimizable_tensors["mirror"]
        self._opacity = optimizable_tensors["opacity"]
        self._scaling = optimizable_tensors["scaling"]
        self._rotation = optimizable_tensors["rotation"]
    ...
    def densification_postfix(self, new_xyz, new_refl, new_mirror, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "refl": new_refl,
        "mirror": new_mirror,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_mirror = self._mirror_weight[selected_pts_mask].repeat(N,1)
        new_opacity = self._opacity[selected_pts_mask].repeat(N,1)

        self.densification_postfix(new_xyz, new_refl, new_mirror, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_mirror = self._mirror_weight[selected_pts_mask]
        new_opacities = self._opacity[selected_pts_mask]
        new_scaling = self._scaling[selected_pts_mask]
        new_rotation = self._rotation[selected_pts_mask]

        self.densification_postfix(new_xyz, new_refl, new_mirror, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation)
