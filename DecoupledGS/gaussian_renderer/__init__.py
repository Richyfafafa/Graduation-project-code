...
# normal: x,3
# all normalized
def reflection(rayd, normal):#反射方向
    refl = rayd - 2*normal*torch.sum(rayd*normal, dim=-1, keepdim=True)#$$\mathbf{R} = \mathbf{I} - 2(\mathbf{I} \cdot \mathbf{n})\mathbf{n}$$
    return refl

def sample_cubemap_color(rays_d, env_map):#rays_d: x,3反射方向
...
def get_trans_color(envmap: torch.Tensor, HWK, R, T):
    # Transmission uses the camera ray direction directly (no mirror reflection).
    rays_d = sample_camera_rays(HWK, R, T)
    return sample_cubemap_color(rays_d, envmap)

def get_refl_color(envmap: torch.Tensor, HWK, R, T, normal_map): #RT W2C, normal_map: x,3, normal direction
    rays_d = sample_camera_rays(HWK, R, T)
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, initial_stage = False, more_debug_infos = False, override_color = None, use_mirror_gate = False, render_geometry = False):
    """
    Render the scene. 
    ...
    geom_maps = {}
    if render_geometry:
        depth_values = pc.get_depth(viewpoint_camera.world_view_transform).repeat(1, 3)
        one_values = torch.ones_like(depth_values)
        zero_bg = torch.zeros_like(bg_map_const)
        with torch.no_grad():
            alpha_map, _ = rasterizer_c3(
                means3D = means3D,
                means2D = means2D,
                shs = None,
                colors_precomp = one_values,
                opacities = opacities,
                scales = scales,
                rotations = rotations,
                cov3D_precomp = None,
                bg_map = zero_bg)
        depth_num, _ = rasterizer_c3(
            means3D = means3D,
            means2D = means2D,
            shs = None,
            colors_precomp = depth_values,
            opacities = opacities,
            scales = scales,
            rotations = rotations,
            cov3D_precomp = None,
    ...
    if override_color is not None:#zi'n新增
    ...
    if use_mirror_gate:
        # 2026-02-16: mirror labels gate where reflection can appear.
        refl_ratio = refl_ratio * pc.get_mirror_weight
