...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, save_ims : bool, eval_split : str):
    with torch.no_grad():
        ...
        if len(views) == 0:
            raise RuntimeError(
                "No views selected for evaluation. "
                "If you want test split, add --eval; "
                "or use --eval_split all to evaluate all available views."
            )

        render_set(dataset.model_path, split_name, scene.loaded_iter, views, gaussians, pipeline, background, save_ims)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.save_images, args.eval_split)
