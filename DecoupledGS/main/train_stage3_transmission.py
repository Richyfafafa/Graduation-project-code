...
def stage3_training(dataset, opt, pipe, args):
    ...
    for step in progress:
        ...
        # 残差：GT 减去已知的基础+反射部分
        residual = (gt - C_wo_trans).detach()   # (3, H, W)，作为拟合目标
        ...
        for i in range(N):
            ...
            # 查询第 i 个窗口的透射 Cubemap（直通视线方向）
            C_trans_i = trans_maps[i].get_color(cam.HWK, cam.R, cam.T)  # (3, H, W)
            strength_i = trans_maps[i].strength                           # scalar

            # 透射贡献
            C_contrib_i = strength_i * C_trans_i                          # (3, H, W)

            # 掩码 L1 损失（仅在该窗口区域）
            mask3 = pixel_mask.float().unsqueeze(0).expand(3, -1, -1)    # (3, H, W)
            loss_i = (C_contrib_i - residual).abs() * mask3
