...
__device__ void EdgeTable(int L, int flag, int * index_xy) {
	// 作用:
	// - 当双线性采样点落到 cubemap 某个面的边界外时，
	//   需要把该采样点映射到相邻面的像素坐标。
	// 输入:
	// - flag: 越界方向编码（u 左/右越界, v 上/下越界）
	// - index_xy: [face, x, y]，会被原地改写到相邻面坐标
...
{
	// 方向向量 -> cubemap 面索引 + 面内 uv
	// 规则:
	// 1) 选择 |x|,|y|,|z| 中最大的轴作为主轴
	// 2) 根据主轴正负确定面索引 (0..5)
	// 3) 在该面局部坐标中计算 uv（范围约为 [-1, 1]）
	int max_dim = 0;
	scalar_t x_ = abs(x);
	scalar_t y_ = abs(y);
	scalar_t z_ = abs(z);
...
__device__ bool Compute_Seamless_Index(
	...
	// seamless 双线性采样的索引构建函数。
	// 给定:
	// - 当前面 index
	// - 面内 uv
	// 计算:
	// - 四个采样点对应的 (face, x, y)（必要时跨面）
	// - 双线性权重参数 kx, ky
	// - 是否是“顶点特殊情况”（同时跨两个边）
	//
	// 输出 index_xy 布局:
	// [p00.face, p00.x, p00.y, p01.face, p01.x, p01.y, p10.face, p10.x, p10.y, p11.face, p11.x, p11.y]
...
__device__ void Compute_Cubemap_UV_Backward(
	...
	// 方向到 uv 映射的反向传播:
	// 输入:
	// - uv[0], uv[1] 存放的是 dL/du, dL/dv（在调用处赋值）
	// 输出:
	// - grad_xyz: dL/dx, dL/dy, dL/dz
	int face = index / 2;
	if (face == 0) {//0,1
...
void cubemap_encode_forward(
	...
	// C++ 侧前向入口:
	// 根据 interp/seamless 路由到不同 CUDA kernel
	CHECK_CUDA(inputs);
	CHECK_CUDA(cubemap);
...
__global__ void Cubemap_Nearest_Backward_Kernel(
	...
	// 反向: 对最近邻采样求梯度
	// 注意最近邻对 uv 不连续，方向梯度在分段常值区域内通常为 0。
	const uint32_t n = blockIdx.x * blockDim.x + threadIdx.x;
	if (n < B){
		scalar_t vx = inputs[n * 3 + 0];
		scalar_t vy = inputs[n * 3 + 1];
...
void cubemap_encode_backward(
	...
	// C++ 侧反向入口:
	// 按 interp/seamless 选择对应 backward kernel
